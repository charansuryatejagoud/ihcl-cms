...
import {IoApps as AppIcon, IoEyeOff as EyeClosedIcon, IoEye as EyeOpenIcon} from 'react-icons/io5'
...
export default function QueryBuilder() {
        ...
        if (data === null) {
          ...
          data &&
            data.events.map(async (doc) => {
              ...
              const identifier = (doc?.basicInfo?.title + '-' + data?.identifier)
                ?.toLowerCase()
                .replace(/ /g, '-')
              let hotel = {
                _key: nanoid(),
  ...
  async function processSeo({
    ...
    documents?.map(async (document) => {
      try {
        ...
        console.error('Oh no, the update failed: ', document?._id, 'Error : ', err.message)
      }
    })
  }
  ...
  return (
    ...
    <Card padding={4}>
      ...
      <TabPanel aria-labelledby="seo-tab" hidden={tabId !== 'seo'} id="seo-panel" paddingTop={4}>
        <Card marginTop={2} radius={2}>
