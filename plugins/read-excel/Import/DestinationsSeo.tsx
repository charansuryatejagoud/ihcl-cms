...
function extractDestinationsSeo({ data, prevData }) {
  const seoData: any = {};
...
function DestinationsSeo({ callBack, getLoader }: ImportComponent) {
  ...
  const [destinationsSeoData, setDestinationsSeoData] = useState([]);
  ...
  const migrateExcelData = async (callBack, UpdateLoader) => {
    ...
    await destinationsSeoData.map(async (destination, index) => {
      await client
        .fetch(
...
function getDestinationDoc(
  { excelData, document = null, type = TYPE_DESTINATION },
  returnObject: any = {},
) {
  ...
  if (excelData?.seoInfo?.length > 0) {
    returnObject.seoInfo = excelData?.seoInfo?.map((seo) => {
      return {
        _key: nanoid(),
        _type: TYPE_DESTINATION_SEO_INFO,
        navigation:
          destinationNavigation?.filter(
            (data) => data.title == seo?.navigation,
          )?.[0]?.value ?? seo?.navigation?.toLowerCase(),
        title: seo?.seoTitle,
