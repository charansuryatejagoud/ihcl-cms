...
import {FeatureSchemaDefinition, SchemaItemsProperties, VariantSchemaProperties} from './types'
...
function extractVariants(features: FeatureSchemaDefinition[]) {
  ...
  return variantDefKeys.reduce((obj: any, key) => {
...
function extractSchemaItems(features: FeatureSchemaDefinition[]) {
  ...
  return schemaItemDefKeys.reduce((allFeatures: any, property) => {
